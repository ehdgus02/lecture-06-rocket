...
function Home() {
    ...
    const [loading, setLoading] = useState(true); // truel, false

    // list라는 state에 초기값을 []로 넣어버리면,
    // 타입스크립트 엔진은 "타입추론"을 통해 추측을 []라고 하는데, 그 안에 들어갈 요소의 타입은 알 수 없음
    // 얘가 안에 들어가는 요소의 타입을 알 수 없기 때문에 결국, never[] 로 타입 추론을 해버림
    const [list, setList] = useState<RocketType[]>([]);

    useEffect(() => {
        // 비동기 함수의 반대는 동기 함수
        // 동기 함수는 : 일반 함수들 -> 원래부터 자바스크립트 엔진이 실행하면 결과가 나온 뒤에 다음 줄을 넘어가는 함수들.
